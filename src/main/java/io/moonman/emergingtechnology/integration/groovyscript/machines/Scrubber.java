...
public class Scrubber extends VirtualizedRegistry<IMachineRecipe> {
    @Override
    @GroovyBlacklist
    public void onReload() {
        ScrubberRecipes.getRecipes().removeAll(removeScripted());
        ScrubberRecipes.getRecipes().addAll(restoreFromBackup());
    }
    ...
    public boolean remove(IMachineRecipe recipe) {
        if (ScrubberRecipes.getRecipes().remove(recipe)) {
            addBackup(recipe);
            return true;
        }
        return false;
    ...
    public boolean removeByOutput(IIngredient output) {
        return ScrubberRecipes.getRecipes().removeIf(recipe -> {
    ...
    public void removeAll() {
        ScrubberRecipes.getRecipes().forEach(this::addBackup);
        ScrubberRecipes.getRecipes().clear();
    }

    @MethodDescription(type = MethodDescription.Type.QUERY)
    public SimpleObjectStream<IMachineRecipe> streamRecipes() {
        return new SimpleObjectStream<>(ScrubberRecipes.getRecipes()).setRemover(this::remove);
    }
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<IMachineRecipe> {
        ...
        public String getErrorMsg() {
            return "Error adding Scrubber recipe";
        }
        ...
        public IMachineRecipe register() {
            ...
            GSPlugin.instance.scrubber.add(recipe);
            return recipe;
        }
    }
}
