...
public class AlgaeBioreactor extends VirtualizedRegistry<IMachineRecipe> {
    @Override
    @GroovyBlacklist
    public void onReload() {
        AlgaeBioreactorRecipes.getRecipes().removeAll(removeScripted());
        AlgaeBioreactorRecipes.getRecipes().addAll(restoreFromBackup());
    }
    ...
    public boolean remove(IMachineRecipe recipe) {
        if (AlgaeBioreactorRecipes.getRecipes().remove(recipe)) {
            addBackup(recipe);
            return true;
        }
        return false;
    ...
    public boolean removeByOutput(IIngredient output) {
        return AlgaeBioreactorRecipes.getRecipes().removeIf(recipe -> {
    ...
    public void removeAll() {
        AlgaeBioreactorRecipes.getRecipes().forEach(this::addBackup);
        AlgaeBioreactorRecipes.getRecipes().clear();
    }

    @MethodDescription(type = MethodDescription.Type.QUERY)
    public SimpleObjectStream<IMachineRecipe> streamRecipes() {
        return new SimpleObjectStream<>(AlgaeBioreactorRecipes.getRecipes()).setRemover(this::remove);
    }
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<IMachineRecipe> {
        ...
        public String getErrorMsg() {
            return "Error adding Algae Bioreactor recipe";
        }
        ...
        public IMachineRecipe register() {
            ...
            GSPlugin.instance.algaeBioreactor.add(recipe);
            return recipe;
        }
    }
}
