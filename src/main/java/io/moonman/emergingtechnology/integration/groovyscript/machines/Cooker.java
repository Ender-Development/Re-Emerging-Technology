...
public class Cooker extends VirtualizedRegistry<IMachineRecipe> {
    @Override
    @GroovyBlacklist
    public void onReload() {
        CookerRecipes.getRecipes().removeAll(removeScripted());
        CookerRecipes.getRecipes().addAll(restoreFromBackup());
    }
    ...
    public boolean remove(IMachineRecipe recipe) {
        if (CookerRecipes.getRecipes().remove(recipe)) {
            addBackup(recipe);
            return true;
        }
        return false;
    ...
    public boolean removeByOutput(IIngredient output) {
        return CookerRecipes.getRecipes().removeIf(recipe -> {
    ...
    public void removeAll() {
        CookerRecipes.getRecipes().forEach(this::addBackup);
        CookerRecipes.getRecipes().clear();
    }

    @MethodDescription(type = MethodDescription.Type.QUERY)
    public SimpleObjectStream<IMachineRecipe> streamRecipes() {
        return new SimpleObjectStream<>(CookerRecipes.getRecipes()).setRemover(this::remove);
    }
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<IMachineRecipe> {
        ...
        public String getErrorMsg() {
            return "Error adding Cooker recipe";
        }
        ...
        public IMachineRecipe register() {
            ...
            GSPlugin.instance.cooker.add(recipe);
            return recipe;
        }
    }
}
