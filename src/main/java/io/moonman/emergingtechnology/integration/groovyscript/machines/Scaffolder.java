...
public class Scaffolder extends VirtualizedRegistry<IMachineRecipe> {
    @Override
    @GroovyBlacklist
    public void onReload() {
        ScaffolderRecipes.getRecipes().removeAll(removeScripted());
        ScaffolderRecipes.getRecipes().addAll(restoreFromBackup());
    }
    ...
    public boolean remove(IMachineRecipe recipe) {
        if (ScaffolderRecipes.getRecipes().remove(recipe)) {
            addBackup(recipe);
            return true;
        }
        return false;
    ...
    public boolean removeByOutput(IIngredient output) {
        return ScaffolderRecipes.getRecipes().removeIf(recipe -> {
    ...
    public void removeAll() {
        ScaffolderRecipes.getRecipes().forEach(this::addBackup);
        ScaffolderRecipes.getRecipes().clear();
    }

    @MethodDescription(type = MethodDescription.Type.QUERY)
    public SimpleObjectStream<IMachineRecipe> streamRecipes() {
        return new SimpleObjectStream<>(ScaffolderRecipes.getRecipes()).setRemover(this::remove);
    }
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<IMachineRecipe> {
        ...
        public String getErrorMsg() {
            return "Error adding Scaffolder recipe";
        }
        ...
        public IMachineRecipe register() {
            ...
            GSPlugin.instance.scaffolder.add(recipe);
            return recipe;
        }
    }
}
