...
public class Shredder extends VirtualizedRegistry<IMachineRecipe> {
    @Override
    @GroovyBlacklist
    public void onReload() {
        ShredderRecipes.getRecipes().removeAll(removeScripted());
        ShredderRecipes.getRecipes().addAll(restoreFromBackup());
    }
    ...
    public boolean remove(IMachineRecipe recipe) {
        if (ShredderRecipes.getRecipes().remove(recipe)) {
            addBackup(recipe);
            return true;
        }
        return false;
    ...
    public boolean removeByOutput(IIngredient output) {
        return ShredderRecipes.getRecipes().removeIf(recipe -> {
    ...
    public void removeAll() {
        ShredderRecipes.getRecipes().forEach(this::addBackup);
        ShredderRecipes.getRecipes().clear();
    }

    @MethodDescription(type = MethodDescription.Type.QUERY)
    public SimpleObjectStream<IMachineRecipe> streamRecipes() {
        return new SimpleObjectStream<>(ShredderRecipes.getRecipes()).setRemover(this::remove);
    }
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<IMachineRecipe> {
        ...
        public String getErrorMsg() {
            return "Error adding Shredder recipe";
        }
        ...
        public IMachineRecipe register() {
            ...
            GSPlugin.instance.shredder.add(recipe);
            return recipe;
        }
    }
}
